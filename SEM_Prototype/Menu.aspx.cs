        ...
        void bindList()
        {
            ...
            if (rblCategory.SelectedIndex != -1)
            {
                switch (ddlArtSort.SelectedIndex)
                {
                    ...
                    case 3:
                        command = new SqlCommand("Select * from Menu " + "WHERE Category = @Category AND Availability = '1' ORDER BY Price ASC", conn);
                        command.Parameters.AddWithValue("@Category", rblCategory.SelectedIndex + 1);
                        dataAdapter.SelectCommand = command;
                        break;
                    ...
                    case 4:
                        command = new SqlCommand("Select * from Menu " + "WHERE Category = @Category AND Availability = '1' ORDER BY Price DESC", conn);
                        command.Parameters.AddWithValue("@Category", rblCategory.SelectedIndex + 1);
                        dataAdapter.SelectCommand = command;
                        break;
            ...
            else
            {
                switch (ddlArtSort.SelectedIndex)
                {
                    ...
                    //Sort by price asscending
                    case 3:
                        dataAdapter = new SqlDataAdapter("Select * from Menu where Availability = '1' ORDER BY Price ASC", conn);
                        break;

                    //Sort by price descending
                    case 4:
                        dataAdapter = new SqlDataAdapter("Select * from Menu where Availability = '1' ORDER BY Price DESC", conn);
                        break;
                }
            }
